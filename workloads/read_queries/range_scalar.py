"""
Range Query on Scalar Field — parameterized by --selectivity.

Single @workload: bounded 30% range filter on scalar_sel{N} with limit=document_count.
Index: created in @post_load on the target field only.

Usage:
  locust -f workloads/range_scalar.py --uri=... --document-count=1000000 --selectivity=100
"""
...
class RangeScalar(FindReadWorkload):
    """Bounded 30% range query on scalar field at specified selectivity."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        if suffix == "unique":
            self._query_name = "range_scalar_unique_30pct_limitN"
        else:
            self._query_name = f"range_scalar_{suffix}_30pct_limitN"
