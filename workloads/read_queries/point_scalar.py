"""
Point Query on Scalar Field — parameterized by --selectivity.

Single @workload: equality filter on scalar_sel{N} with limit=N.
Index: created in @post_load on the target field only.

Usage:
  locust -f workloads/point_scalar.py --uri=... --document-count=1000000 --selectivity=100
"""
...
class PointScalar(FindReadWorkload):
    """Point query on scalar field at specified selectivity."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._field_name = f"scalar_{suffix}"
        self._field = getattr(self, self._field_name)
        self._limit = self._sel_value

        if suffix == "unique":
            self._query_name = "point_scalar_unique"
        else:
            self._query_name = f"point_scalar_{suffix}_limit{self._sel_value}"

    @post_load
    def setup_index(self):
        """Drop all indexes, create single index on target field.
    ...
    def on_start(self):
        super().on_start()
        # Update workload name for Locust metrics
